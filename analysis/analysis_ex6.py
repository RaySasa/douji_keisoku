# 同時測定の角度分布(円柱) fitting + theta依存補正(ガウス)
...
def correction(theta_deg, alpha):
    ...
    return np.exp(-alpha * np.sin(theta)**2)
...
plt.savefig("tex/analysis_ex6_fit_corrected.pdf", dpi=300, bbox_inches="tight")
plt.show()