# 同時測定の角度分布(円柱) fitting + cos^beta補正
...
def Fc(x):
    return (
        x * np.sqrt(np.abs(Rb**2 - x**2))
        + Rb**2 * safe_arcsin(x / Rb)
    )


def Fe(x, xT, a, b):
    return b / a * (
        (x - xT) * np.sqrt(np.abs(a**2 - (x - xT)**2))
        + a**2 * safe_arcsin((x - xT) / a)
    )
...
def calc_distribution(theta_deg):
    ...
    for th in theta_rad:
        ...
        yT = (
            yhp * (c + L * np.cos(th)) / xhp
            - L * np.sin(th)
        )

        zT = (
            zhp * (c + L * np.cos(th)) / xhp
        )
...
def correction(theta_deg, beta):

    theta = np.radians(theta_deg)

    return np.cos(theta)**beta
...
def fit_func(theta_deg, N0, beta):
...
N0_fit, beta_fit = popt

N0_err, beta_err = np.sqrt(np.diag(pcov))

print(f"N0 = {N0_fit:.6g} ± {N0_err:.6g}")
print(f"beta = {beta_fit:.6g} ± {beta_err:.6g}")
...
plt.plot(
    theta_plot,
    y_fit,
    label=r"$\cos^\beta\theta$ corrected fit"
)
...
plt.savefig(
    "tex/analysis_ex7_fit_cosbeta.pdf",
    dpi=300,
    bbox_inches="tight"
)
